...
chokidar
  .watch([resolve(__dirname, '../src/components/*/README.md'), resolve(__dirname, '../src/components/*/demo/*')])
  .on('all', (event, path) => {
    updateMdByPath(event, path)
      .then(() => {
        if ((event === 'change' || (event === 'add' && path.endsWith('index.md'))) && browserSync.reload) {
