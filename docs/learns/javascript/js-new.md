...
由此可以看出：这里 Student 函数中的 this 指向 new Student()生成的对象 student。同时也进一步证实了小结 1，即：这个新创建的对象会被执行 Student 方法（student.**proto**）
...
前面六种基本类型都会正常返回{name: '轩辕 Rowboat'}，后面的 Object(包含 Functoin, Array, Date, RegExg, Error)都会直接返回这些值。
